...
void LIST(void) ;
	...
	private :
			...
			int   code, dd, mm, yy ;
			char  name[50], address[100], phone[12], desig[16] ;
...
//**********************************************************
void EMPLOYEE :: ADD_RECORD(int ecode, char ename[50], char eaddress[100], char ephone[12], int d, int m, int y, char edesig[16], char egrade, char ehouse, char econv, float eloan, float ebasic)
{
	fstream file ;
...
// EMPLOYEE'S FILE
//**********************************************************
void EMPLOYEE :: MODIFY_RECORD(int ecode, char ename[50], char eaddress[100], char ephone[12], char edesig[16], char egrade, char ehouse, char econv, float eloan, float ebasic)
{
	int recno ;
...
void EMPLOYEE :: NEW_EMPLOYEE(void)
{
	...
	char  ename[50], eaddress[100], ephone[12], edesig[16], t1[10] ;
...
cout <<"Designation  : " ;
	...
	do
	{
		...
		if (ename[0] == '0')
			return ;
		if (strlen(ename) < 1 || strlen(ename) > 50)
		{
			valid = 0 ;
			gotoxy(5,25) ; clreol() ;
			cout <<"\7Enter correctly (Range: 1..50)" ;
			getch() ;
		}
	} while (!valid) ;
	do
...
return ;
		if (strlen(eaddress) < 1 || strlen(eaddress) > 100)
		{
			valid = 0 ;
			gotoxy(5,25) ; clreol() ;
			cout <<"\7Enter correctly (Range: 1..100)" ;
			getch() ;
		}
...
void main()
{
	 clrscr();
	 gotoxy(27,10);
	 cout<<"Shivam Mani Tripathi";

	 gotoxy(27,11);
	 cout<<"COMPUTER SCIENCE ";
	 gotoxy(27,12);
	 cout<<"PROJECT:- SALARY MANAGMENT";
	 int i,j,k;
 char ch[100];
 gotoxy(24,18);
 i=18;
